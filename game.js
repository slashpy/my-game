...
(function() {
	...
	// Starfield
	let starColor = '#93c5fd';
	let levelTint = 'rgba(255,255,255,0.04)';
	let paddleColor = '#fbcfe8';
	let ballPalette = ['#f9a8d4','#bfdbfe','#fde68a','#c4b5fd','#a7f3d0'];
	const stars = [];
	for (let i = 0; i < 120; i++) {
	...
	// Skin and Level state
	let currentSkin = localStorage.getItem('skin') || 'pink';
	let currentLevel = localStorage.getItem('level') || 'night';

	function applySkin(skin) {
		currentSkin = skin;
		localStorage.setItem('skin', skin);
		// paddle color by skin
		if (skin === 'mint') paddleColor = '#a7f3d0';
		else if (skin === 'lavender') paddleColor = '#c4b5fd';
		else if (skin === 'gold') paddleColor = '#fde68a';
		else paddleColor = '#fbcfe8';
		// ball palette
		if (skin === 'mint') ballPalette = ['#a7f3d0','#bfdbfe','#86efac','#fef3c7','#ddd6fe'];
		else if (skin === 'lavender') ballPalette = ['#c4b5fd','#f9a8d4','#a7f3d0','#bfdbfe','#fde68a'];
		else if (skin === 'gold') ballPalette = ['#fde68a','#fbbf24','#f9a8d4','#bfdbfe','#c4b5fd'];
		else ballPalette = ['#f9a8d4','#bfdbfe','#fde68a','#c4b5fd','#a7f3d0'];
	}
	...
	function applyLevel(level) {
		...
		if (level === 'sakura') {
			levelTint = 'rgba(255,192,203,0.08)';
			starColor = '#fecdd3';
			currentBallMinSpeed = BASE_BALL_MIN_SPEED * 0.95;
			currentBallMaxSpeed = BASE_BALL_MAX_SPEED * 0.95;
		} else if (level === 'nebula') {
			levelTint = 'rgba(147,197,253,0.08)';
			starColor = '#a5b4fc';
			currentBallMinSpeed = BASE_BALL_MIN_SPEED * 1.05;
			currentBallMaxSpeed = BASE_BALL_MAX_SPEED * 1.1;
		} else {
			levelTint = 'rgba(255,255,255,0.04)';
			starColor = '#93c5fd';
	...
	function spawnBall(now) {
		...
		const color = choice(ballPalette);
	...
	function showPanel(which) {
		if (panelPlay) panelPlay.style.display = which === 'play' ? '' : 'none';
		if (panelSkins) panelSkins.style.display = which === 'skins' ? '' : 'none';
		if (panelLevels) panelLevels.style.display = which === 'levels' ? '' : 'none';
	}

	if (tabPlay) tabPlay.addEventListener('click', () => showPanel('play'));
	if (tabSkins) tabSkins.addEventListener('click', () => showPanel('skins'));
	if (tabLevels) tabLevels.addEventListener('click', () => showPanel('levels'));

	// Skin selection
	Array.from(document.querySelectorAll('.skin')).forEach((btn) => {
	...
	// Level selection
	Array.from(document.querySelectorAll('input[name="level"]')).forEach((r) => {
		r.addEventListener('change', (e) => {
			if (e.target.checked) applyLevel(e.target.value);
		});
	});
	...
	resetGame();
	overlayTitle.textContent = 'Kitty Catch';
	overlaySub.textContent = 'Tap Play to start';
